...
int main(int argc, char const *argv[])
{
    ...
    int32_t ** matrix = converter::convert_las_to_matrix(las_filename, DESIRED_MATRIX_SIZE, DESIRED_MATRIX_SIZE);

    // write output to file
    std::string out_filename(las_filename.substr(0, las_filename.size() - 3) + "data");
    converter::write_matrix_to_file(out_filename, matrix, DESIRED_MATRIX_SIZE, DESIRED_MATRIX_SIZE);
